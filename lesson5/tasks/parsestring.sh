...
find . -name "*.html" -type f | while read file; do
    ...
    while IFS= read -r line; do
        ...
        if [[ $line =~ if\ \(matchedPairs[[:space:]]*===?[[:space:]]*([0-9]+)[[:space:]]*\)\ *{ ]]; then
            number="${BASH_REMATCH[1]}"
            echo "✅ Найдено в $file: if (matchedPairs === $number) {"
            echo "   Заменено на: if (matchedPairs === pairs.length) {"
            # Используем sed для безопасной замены в строке
            echo "$line" | sed 's/if (matchedPairs[[:space:]]*===\?[[:space:]]*[0-9][[:space:]]*) {/if (matchedPairs === pairs.length) {/' >> "$temp_file"
            changed=1
        else
            echo "$line" >> "$temp_file"
        fi
